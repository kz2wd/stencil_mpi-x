...
static void stencil_init(void)
{
	...
	for (int y = -1; y < HALO_SIZE + 1; y++) {
		...
		FIRST_ROW[size_x - 1 + size_x * y] =
		    size_y - (y + HALO_SIZE * rank) - 1;
	}
...
#pragma omp parallel for schedule(static)
	for (int y = start; y < end; y++) {
		for (int x = 1; x < size_x - 1; x++) {
			...
			if (convergence &&
			    (fabs(prev_values[x + size_x * y] - values[x + size_x * y]) >
			        epsilon)) {
				convergence = 0;
			}
		}
...
static void handle_top_neighbor()
{
	...
	MPI_Isend(FIRST_ROW, size_x, MPI_FLOAT, rank - 1, tag, MPI_COMM_WORLD,
	    &requests[0]);
	MPI_Irecv(TOP_HALO, size_x, MPI_FLOAT, rank - 1, tag, MPI_COMM_WORLD,
	    &requests[1]);
...
static void handle_bot_neighbor()
{
	...
	MPI_Status statuses[2];
	MPI_Request requests[2];
	MPI_Isend(LAST_ROW, size_x, MPI_FLOAT, rank + 1, tag, MPI_COMM_WORLD,
	    &requests[1]);
	MPI_Irecv(BOT_HALO, size_x, MPI_FLOAT, rank + 1, tag, MPI_COMM_WORLD,
	    &requests[0]);
...
{
	MPI_Allreduce(local_convergence, local_convergence, 1, MPI_INT, MPI_LAND,
	    MPI_COMM_WORLD);
}
...
{
	if (STENCIL_SIZE > 10 || HALO_SIZE > 10)
		return;
	MPI_Barrier(MPI_COMM_WORLD);
